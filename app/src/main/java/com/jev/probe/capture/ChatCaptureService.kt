...
open class ChatCaptureService : AccessibilityService() {
    ...
    private fun maybeCapture() {
        ...
        lastSignature = sig
        // sides + kind + lengths only — never message content
        Log.d(TAG, "snapshot[$pkg] title=${snapshot.title} n=${snapshot.messages.size} " +
            snapshot.messages.takeLast(6).joinToString(" | ") {
                "${it.side}:${it.kind}:${it.text.length}"
            })
        if (snapshot.latestIsOtherWithdraw) {
            Log.i(TAG, "event[$pkg] other_withdraw")
        }
        ...
        // Other-person withdraw tips count as latestFrom=other (see WeChatAdapter).
     ...
     * — except WeChat withdraw tips, which carry 你/对方 in the tip text itself.
     */
    private fun groupOcrLines(lines: List<OcrLine>): List<Msg> {
        ...
        fun flush() {
            ...
            out.add(classifyOcrBubble(buf.toString()))
            buf.setLength(0)
        }
        for (l in usable) {
            val p = prev
            ...
            // A withdraw tip is a single centered line — flush it alone so it
            // is not glued to the previous bubble.
            val piece = l.text.trim()
            if (parseWeChatWithdraw(piece) != null) {
                flush()
                out.add(classifyOcrBubble(piece))
                prev = l
                continue
            }
    ...
    /** Tag OCR bubble text as withdraw / image / plain other-side text. */
    private fun classifyOcrBubble(raw: String): Msg {
        parseWeChatWithdraw(raw)?.let { (side, label) ->
            return Msg(side, label, Msg.KIND_WITHDRAW)
        }
        if (isWeChatImageLabel(raw)) return Msg("other", raw, Msg.KIND_IMAGE)
        return Msg("other", raw, Msg.KIND_TEXT)
    }
    ...
    private fun finishOcrSnapshot(snapshot: ChatSnapshot, pkg: String, manual: Boolean) {
        ...
        // Withdraw tip wins over the generic "OCR 未分边" caveat when both apply.
        val annotated = when {
            snapshot.latestIsOtherWithdraw ->
                snapshot.copy(note = "检测到对方撤回了一条消息（文字/图片）")
            snapshot.messages.lastOrNull()?.kind == Msg.KIND_WITHDRAW ->
                snapshot.copy(note = "检测到你撤回了一条消息")
            else -> snapshot
        }
        currentSnapshot = annotated
        val sig = annotated.signature()
        ...
        if (!manual && sig == lastSignature) {
            if (overlay?.isShowing() != true) {
                overlay?.setNote(annotated.note)
                overlay?.showIdle(annotated.title)
            }
            return
        }
        ...
        lastSignature = sig
        if (annotated.latestIsOtherWithdraw) Log.i(TAG, "event[$pkg] other_withdraw ocr")

        val auto = prefs.ocrAutoAnalyze && prefs.autoAnalyze && annotated.latestFrom == "other"
        if (manual || auto) {
            pendingSnapshot = annotated
            main.removeCallbacks(debounce)
            runAnalysis()
        } else {
            overlay?.setNote(annotated.note)
