...
/** Exact WeChat withdraw tip; who-prefix is 你 / 对方 / a short display name. */
private val WECHAT_WITHDRAW_TIP = Regex("""^(你|对方|.{1,16})撤回了一条消息$""")

/**
 * WeChat system tip "你/对方/某人撤回了一条消息". Exact match after trimming a
 * trailing "重新编辑" affordance that sometimes sits in the same TextView.
 * Returns (side, displayText) or null when [raw] is not a withdraw tip.
 */
internal fun parseWeChatWithdraw(raw: String): Pair<String, String>? {
    var t = raw.trim()
    if (t.endsWith("重新编辑")) t = t.removeSuffix("重新编辑").trim()
    if (t.length > 40 || !t.endsWith("撤回了一条消息")) return null
    if (!WECHAT_WITHDRAW_TIP.matches(t)) return null
    val side = when {
        t.startsWith("你撤回") -> "me"
        else -> "other" // "对方撤回…" or "张三撤回…"
    }
    return side to "[撤回] $t"
}

/** True when a bubble label is WeChat's image placeholder (text or desc). */
internal fun isWeChatImageLabel(label: String?): Boolean {
    if (label.isNullOrBlank()) return false
    val t = label.trim()
    return t == "[图片]" || t == "[Image]" || t.equals("图片", ignoreCase = true) ||
        t.equals("Image", ignoreCase = true)
}
 ...
 *
 *  Also collects centered "…撤回了一条消息" system tips (not `id/bkl`) and
 *  image bubbles whose `text` is empty but contentDescription says 图片 —
 *  so a withdraw of either a text or image row becomes a detectable event.
 *
...
class WeChatAdapter : ChatAppAdapter {
    ...
    override fun extract(root: AccessibilityNodeInfo, res: Resources): ChatSnapshot? {
        ...
        val minCenterX = (width * 0.25).toInt()
        val maxCenterX = (width * 0.75).toInt()
        // top, Msg — bubbles and withdraw tips share one timeline
        val rows = ArrayList<Pair<Int, Msg>>()
        ...
        while (stack.isNotEmpty() && guard < 5000) {
            ...
            if (id == BUBBLE_ID) {
                ...
                val msg = weChatBubbleMsg(text, desc, b.centerX(), width)
                if (msg != null) {
                    rows.add(b.top to msg)
                    if (b.top < firstBubbleTop) firstBubbleTop = b.top
                }
            } else if (!text.isNullOrBlank()) {
                // System tips (withdraw) are not bkl; take short centered lines only.
                parseWeChatWithdraw(text)?.let { (side, label) ->
                    val b = Rect(); node.getBoundsInScreen(b)
                    if (b.bottom > topBand && b.top < bottomBand &&
                        b.centerX() in minCenterX..maxCenterX
                    ) {
                        rows.add(b.top to Msg(side, label, Msg.KIND_WITHDRAW))
                    }
                }
            }
            for (i in node.childCount - 1 downTo 0) node.getChild(i)?.let { stack.addLast(it) }
        ...
        if (rows.isEmpty()) return if (isChat) ChatSnapshot(title, emptyList()) else null
        rows.sortBy { it.first }
        val msgs = rows.map { it.second }
        val note = when {
            msgs.lastOrNull()?.let { it.kind == Msg.KIND_WITHDRAW && it.side == "other" } == true ->
                "检测到对方撤回了一条消息（文字/图片）"
            msgs.lastOrNull()?.let { it.kind == Msg.KIND_WITHDRAW && it.side == "me" } == true ->
                "检测到你撤回了一条消息"
            else -> null
        }
        return ChatSnapshot(title, msgs, note = note)
    }

    /**
     * Turn one `id/bkl` node into a [Msg], or null when there is neither readable
     * text nor an image placeholder signal.
     */
    private fun weChatBubbleMsg(
        text: String?,
        desc: String?,
        centerX: Int,
        width: Int
    ): Msg? {
        val side = if (centerX > width / 2) "me" else "other"
        val body = text?.trim().orEmpty()
        if (body.isNotEmpty()) {
            val kind = if (isWeChatImageLabel(body)) Msg.KIND_IMAGE else Msg.KIND_TEXT
            return Msg(side, body, kind)
        }
        // Image bubbles often expose "图片" only on contentDescription.
        if (isWeChatImageLabel(desc)) {
            return Msg(side, "[图片]", Msg.KIND_IMAGE)
        }
        return null
    }
